  ...
  return (
    <div>
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center justify-between py-4 text-left">
        ...
        <span className={`h-6 w-6 grid place-items-center rounded-full ${open ? 'bg-primary-600 text-white' : 'text-slate-500'}`}>{open ? '-' : '+'}</span>
      </button>
  ...
  return (
    <section className="section-y relative">
      ...
      <div className="container-x">
        ...
        <div className="mx-auto mt-6 max-w-2xl rounded-2xl bg-white p-4 md:p-6 shadow-card">
          {faqs.map((f, i) => (
